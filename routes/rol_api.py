...
async def get_roles_active():
    try:
        return await get_list_roles()
    except Exception as ex:
        return exit_json(0, {"success": False, "mensaje": str(ex)})


@router.get("/List/{idRol}")
async def get_roles_active_by_id(idRol: int, all_permissions: bool = Query(False), only_permissions: bool = Query(False)):
    try:
        return await get_list_roles_by_id(idRol, all_permissions, only_permissions)
...
async def post_create_new_rol(
    ...
    try:
        ...
        return await create_new_roles(createRol, current_user)
    except Exception as ex:
        return exit_json(0, {"success": False, "mensaje": str(ex)})
...
async def put_update_active_role(
    ...
    try:
        ...
        return await put_update_role(updateRol, current_user)
    except Exception as ex:
        return exit_json(0, {"success": False, "mensaje": str(ex)})
